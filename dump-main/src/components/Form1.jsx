  ...
  const handleOnline = async () => {
    ...
    if (offlineForms.length > 0) {
      ...
      for (const form of offlineForms) {
        ...
        try {
          ...
          if ('data' in form && 'file' in form && '_id' in form) {
    ...
    const syncDataToMongoDB = async (formData, file) => {
      try {
        ...
        const response = await fetch('http://localhost:5000/check_collection_empty');
        ...
        // console.log('reading',formData.data.timestamp)
        console.log(formData.timestamp)

       
        
        // Process formData.data.timestamp if it exists
        if (formData.timestamp) {
            const additionalTimestamp = formData.timestamp;
            const additionalDate = new Date(additionalTimestamp);
            formattedDateString = additionalDate.toUTCString();
            console.log(formattedDateString);
        
            // You can use additionalTimestamp as needed
        }
        else{
          
          const timestamp = formData.data.timestamp;
          const date = new Date(timestamp);
          formattedDateString = date.toUTCString();
          console.log(formattedDateString);
